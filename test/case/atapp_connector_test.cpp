...
CASE_TEST(atapp_connector, get_address_type) {
  ...
  CASE_EXPECT_TRUE(
      !(app.get_address_type("unix:///tmp/path.sock") & atapp::app::address_type_t::EN_ACAT_LOCAL_PROCESS));

  CASE_EXPECT_TRUE(!(app.get_address_type("ipv4://127.0.0.1:1234") & atapp::app::address_type_t::EN_ACAT_LOCAL_HOST));
  CASE_EXPECT_TRUE(
      !(app.get_address_type("ipv4://127.0.0.1:1234") & atapp::app::address_type_t::EN_ACAT_LOCAL_PROCESS));
