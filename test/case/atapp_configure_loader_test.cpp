...
CASE_TEST(atapp_configure, load_yaml) {
    ...
    if (!util::file_system::is_exist(conf_path.c_str())) {
        CASE_MSG_INFO() << CASE_MSG_FCOLOR(YELLOW) << conf_path << " not found, skip atapp_configure.load_yaml" << std::endl;
        return;
    }
...
CASE_TEST(atapp_configure, load_conf) {
    ...
    if (!util::file_system::is_exist(conf_path.c_str())) {
        CASE_MSG_INFO() << CASE_MSG_FCOLOR(YELLOW) << conf_path << " not found, skip atapp_configure.load_conf" << std::endl;
        return;
    }
