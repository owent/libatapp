...
namespace atapp {
    LIBATAPP_MACRO_API atapp_endpoint::atapp_endpoint(app &owner, construct_helper_t &)
        : closing_(false), owner_(&owner), pending_message_size_(0)
    ...
    LIBATAPP_MACRO_API const etcd_discovery_node::ptr_t &atapp_endpoint::get_discovery() const UTIL_CONFIG_NOEXCEPT { return discovery_; }
    ...
    LIBATAPP_MACRO_API int32_t atapp_endpoint::retry_pending_messages(const util::time::time_utility::raw_time_t &tick_time,
        ...
        while (!pending_message_.empty()) {
            ...
            if (max_count > 0 && NULL != handle && NULL != connector) {
                --max_count;
                res =
                    connector->on_send_forward_request(handle, msg.type, &msg.msg_sequence, reinterpret_cast<const void *>(msg.data.data()),
                                                       msg.data.size(), msg.metadata.get());
    ...
    void atapp_endpoint::cancel_pending_messages() {
        ...
        while (!pending_message_.empty()) {
            ...
            connector->on_receive_forward_response(handle, msg.type, msg.msg_sequence, EN_ATBUS_ERR_CLOSING,
                                                   reinterpret_cast<const void *>(msg.data.data()), msg.data.size(), msg.metadata.get());
