...
namespace atapp {
    LIBATAPP_MACRO_API atapp_endpoint::atapp_endpoint(app& owner, construct_helper_t&): closing_(false), owner_(&owner), pending_message_size_(0)
    ...
    LIBATAPP_MACRO_API void atapp_endpoint::update_discovery(const etcd_discovery_node::ptr_t& discovery) UTIL_CONFIG_NOEXCEPT {
    ...
    LIBATAPP_MACRO_API int32_t atapp_endpoint::push_forward_message(int32_t type, uint64_t& msg_sequence, const void* data, size_t data_size, const atapp::protocol::atapp_metadata* metadata) {
        // Closing
        if (closing_ || NULL == owner_) {
            do {
                ...
                connector->on_receive_forward_response(handle, type, msg_sequence, EN_ATBUS_ERR_CLOSING, data, data_size, metadata);
            } while (false);
    ...
    LIBATAPP_MACRO_API int32_t atapp_endpoint::retry_pending_messages(const util::time::time_utility::raw_time_t& tick_time, int32_t max_count) {
        // Including equal
        ...
        while (!pending_message_.empty()) {
            ...
            if (max_count > 0 && NULL != handle && NULL != connector) {
                -- max_count;
                res = connector->on_send_forward_request(handle,
                    msg.type, &msg.msg_sequence, 
    ...
    void atapp_endpoint::cancel_pending_messages() {
        ...
        while (!pending_message_.empty()) {
            ...
            connector->on_receive_forward_response(handle,
                msg.type, msg.msg_sequence, EN_ATBUS_ERR_CLOSING,
                reinterpret_cast<const void*>(msg.data.data()), msg.data.size(), 
                msg.metadata.get());
