...
namespace atapp {
    ...
    LIBATAPP_MACRO_API const char* atapp_connector_atbus::name() UTIL_CONFIG_NOEXCEPT {
        return "atapp::connector.atapp";
    }

    LIBATAPP_MACRO_API uint32_t atapp_connector_atbus::get_address_type(const atbus::channel::channel_address_t& addr) const {
        uint32_t ret = 0;
    ...
    LIBATAPP_MACRO_API int32_t atapp_connector_atbus::on_start_listen(const etcd_discovery_node* , const atbus::channel::channel_address_t& addr) {
        if (NULL == get_owner()) {
    ...
    LIBATAPP_MACRO_API int32_t atapp_connector_atbus::on_start_connect(const etcd_discovery_node* discovery, const atbus::channel::channel_address_t& addr, const atapp_connection_handle::ptr_t& handle) {
        if (NULL == get_owner()) {
            return EN_ATAPP_ERR_NOT_INITED;
        }
    ...
    LIBATAPP_MACRO_API int32_t atapp_connector_atbus::on_close_connect(atapp_connection_handle& handle) {
        // remove handle
        LIBATFRAME_UTILS_AUTO_SELETC_MAP(uint64_t, atapp_connection_handle::ptr_t)::iterator iter = handles_.find(handle.get_private_data_u64());
    ...
    LIBATAPP_MACRO_API int32_t atapp_connector_atbus::on_send_forward_request(atapp_connection_handle* handle, int32_t type, uint64_t* sequence,
        const void* data, size_t data_size, const atapp::protocol::atapp_metadata*) {
    ...
    LIBATAPP_MACRO_API void atapp_connector_atbus::on_receive_forward_response(uint64_t app_id, int32_t type, uint64_t msg_sequence, int32_t error_code,
        ...
        if (iter != handles_.end()) {
            atapp_connector_impl::on_receive_forward_response(iter->second.get(), type, msg_sequence, error_code, data, data_size, metadata);
            return;
        }
    ...
    LIBATAPP_MACRO_API void atapp_connector_atbus::on_discovery_event(etcd_discovery_action_t::type, const etcd_discovery_node::ptr_t &) {}
}
