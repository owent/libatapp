...
namespace atapp {
    ...
    LIBATAPP_MACRO_API int32_t atapp_connector_atbus::on_start_connect(const etcd_discovery_node *discovery,
        ...
        if (atbus::channel::is_local_process_address(addr.address.c_str()) && node->get_pid() != discovery->get_discovery_info().pid()) {
            return EN_ATBUS_ERR_CHANNEL_NOT_SUPPORT;
        }
    ...
    LIBATAPP_MACRO_API int32_t atapp_connector_atbus::on_send_forward_request(atapp_connection_handle *handle, int32_t type,
                                                                              uint64_t *sequence, const void *data, size_t data_size,
    ...
    LIBATAPP_MACRO_API void atapp_connector_atbus::on_receive_forward_response(uint64_t app_id, int32_t type, uint64_t msg_sequence,
        ...
        if (iter != handles_.end()) {
            atapp_connector_impl::on_receive_forward_response(iter->second.get(), type, msg_sequence, error_code, data, data_size,
                                                              metadata);
            return;
        }
