 ...
 * @note KeyValue: { "key": "KEY", "create_revision": "number", "mod_revision": "number", "version": "number", "value":
 * "", "lease": "number" } Get data => curl http://localhost:2379/v3/kv/range -X POST -d '{"key": "KEY", "range_end":
 * ""}' # Response {"kvs":
 * [{...}], "more": "bool", "count": "COUNT"} Set data => curl http://localhost:2379/v3/kv/put -X POST -d '{"key":
 * "KEY", "value": "", "lease": "number", "prev_kv": "bool"}' Renew data => curl http://localhost:2379/v3/kv/put -X POST
 * -d '{"key": "KEY", "value": "", "prev_kv": "bool", "ignore_lease": true}' # Response {"header":{...}, "prev_kv":
 * {...}} Delete data => curl http://localhost:2379/v3/kv/deleterange -X POST -d '{"key": "KEY", "range_end": "",
 * "prev_kv": "bool"}' # Response {"header":{...}, "deleted": "number", "prev_kvs": [{...}]}
 *
 *   Watch => curl http://localhost:2379/v3/watch -XPOST -d '{"create_request":  {"key": "WATCH KEY", "range_end": "",
 * "prev_kv": true}
 * }' # Response {"header":{...},"watch_id":"ID","created":"bool", "canceled": "bool", "compact_revision": "REVISION",
 * "events":
 ...
 *       # Response {"header":{...},"members":[{"ID":"ID","name":"NAME","peerURLs":["peer url"],"clientURLs":["client
 * url"]}]}
 *
...
#elif defined(__DragonFly__) || defined(__FreeBSD__) || defined(__FreeBSD_kernel__) || defined(__OpenBSD__) || \
    defined(__NetBSD__)
...
LIBATAPP_MACRO_API etcd_cluster::etcd_cluster()
  ...
  conf_.ssl_min_version = ssl_version_t::type::kDisabled;
...
LIBATAPP_MACRO_API int etcd_cluster::tick() {
  ...
  if (check_flag(flag_t::type::kClosing)) {
    return 0;
  }
  ...
  // keepalive lease
  if (check_flag(flag_t::type::kEnableLease)) {
    if (0 == get_lease()) {
      ret += create_request_lease_grant() ? 1 : 0;

      // run actions after lease granted
      return ret;
    } else if (app::get_sys_now() > conf_.keepalive_next_update_time) {
      ret += create_request_lease_keepalive() ? 1 : 0;
    }
  } else if (!check_flag(flag_t::type::kRunning)) {
  ...
  // resolve ready flag
  if (!check_flag(flag_t::type::kReady) && check_flag(flag_t::type::kRunning)) {
    resolve_ready();
  }
...
LIBATAPP_MACRO_API void etcd_cluster::resolve_ready() noexcept {
  ...
  if (ready) {
    set_flag(flag_t::type::kReady, true);
  }
}
...
LIBATAPP_MACRO_API void etcd_cluster::set_logger(const atfw::util::log::log_wrapper::ptr_t &logger,
  ...
  if (logger_ && !check_flag(flag_t::type::kReady)) {
...
LIBATAPP_MACRO_API bool etcd_cluster::add_keepalive(const std::shared_ptr<etcd_keepalive> &keepalive) {
  ...
  set_flag(flag_t::type::kEnableLease, true);
  ...
  if (check_flag(flag_t::type::kRunning) && 0 != get_lease()) {
    keepalive->active();
  }
  return true;
}
...
LIBATAPP_MACRO_API bool etcd_cluster::add_retry_keepalive(const std::shared_ptr<etcd_keepalive> &keepalive) {
  ...
  if (keepalive_retry_actors_.end() !=
      std::find(keepalive_retry_actors_.begin(), keepalive_retry_actors_.end(), keepalive)) {
    return false;
  }
...
int etcd_cluster::libcurl_callback_on_auth_user_get(atfw::util::network::http_request &req) {
  ...
  do {
    ...
    for (rapidjson::Document::Array::ConstValueIterator role_iter = roles.Begin(); role_iter != roles.End();
         ++role_iter) {
...
bool etcd_cluster::retry_request_member_update(const std::string &bad_url) {
  ...
  if (!check_flag(flag_t::type::kRunning) &&
...
bool etcd_cluster::create_request_lease_grant() {
  ...
  if (check_flag(flag_t::type::kClosing) || !check_flag(flag_t::type::kEnableLease)) {
    return false;
  }
  ...
  if (check_flag(flag_t::type::kRunning) && conf_.keepalive_interval > std::chrono::system_clock::duration::zero()) {
...
int etcd_cluster::libcurl_callback_on_lease_keepalive(atfw::util::network::http_request &req) {
  ...
  do {
    ...
    self->add_stats_success_request();
    if (!self->check_flag(flag_t::type::kRunning) && !self->check_flag(flag_t::type::kClosing)) {
      self->set_flag(flag_t::type::kRunning, true);
    }
...
LIBATAPP_MACRO_API atfw::util::network::http_request::ptr_t etcd_cluster::create_request_kv_get(
  ...
  if (!curl_multi_ || conf_.path_node.empty() || check_flag(flag_t::type::kClosing)) {
    return atfw::util::network::http_request::ptr_t();
  }
...
LIBATAPP_MACRO_API etcd_cluster::on_event_up_down_handle_t etcd_cluster::add_on_event_up(on_event_up_down_fn_t fn,
  ...
  if (trigger_if_running && check_flag(flag_t::type::kRunning)) {
    fn(*this);
  }
...
LIBATAPP_MACRO_API etcd_cluster::on_event_up_down_handle_t etcd_cluster::add_on_event_down(
  ...
  if (trigger_if_not_running && !check_flag(flag_t::type::kRunning)) {
    fn(*this);
  }
...
void etcd_cluster::check_socket_error_code(int socket_code) {
  ...
  if (CURLE_OPERATION_TIMEDOUT == socket_code) {
    set_flag(flag_t::type::kPreviousRequestTimeout, true);
  }
}
...
LIBATAPP_MACRO_API void etcd_cluster::setup_http_request(atfw::util::network::http_request::ptr_t &req,
  ...
  if (check_flag(flag_t::type::kPreviousRequestTimeout)) {
    ...
    set_flag(flag_t::type::kPreviousRequestTimeout, false);
  }
  ...
  if (ssl_version_t::type::kDisabled != conf_.ssl_min_version) {
    ...
    switch (conf_.ssl_min_version) {
      ...
      case ssl_version_t::type::kTlsV12:
...
#  endif
        break;
      case ssl_version_t::type::kTlsV11:
...
#  endif
      ...
      default:
#  if LIBCURL_VERSION_NUM >= 0x072200
        conf_.ssl_min_version = ssl_version_t::type::kTlsV12;
        req->set_opt_long(CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
#    if LIBCURL_VERSION_NUM >= 0x073400
        req->set_opt_long(CURLOPT_PROXY_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
#    endif
