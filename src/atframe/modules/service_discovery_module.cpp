...
LIBATAPP_MACRO_API service_discovery_module::service_discovery_module()
...
LIBATAPP_MACRO_API int service_discovery_module::init() {
  return get_app()->get_etcd_module()->init_cluster(
      cluster_context_.etcd_cluster_holder_,
      [](atframework::atapp::app &app, atframework::atapp::etcd_cluster_holder &cluster_holder) {
        app.get_etcd_module()->load_cluster_conf(cluster_holder, app.get_origin_configure().service_discovery_etcd(),
                                                 nullptr);
      },
      [this](atframework::atapp::app &app, atframework::atapp::etcd_cluster_holder &cluster_holder, bool init) -> int {
        int ret = init_keepalives(cluster_context_);
        if (ret != 0) {
          return ret;
        }
        ret = init_watchers(cluster_context_);
        if (ret != 0) {
          return ret;
        }

        if (init) {
          const std::list<etcd_keepalive::ptr_t> *keepalive_actors[] = {
              &cluster_context_.internal_discovery_keepalive_actors_,
              &cluster_context_.internal_topology_keepalive_actors_};

          if (!etcd_module::check_keepalive_actor_start_success(app, cluster_holder,
                                                                gsl::make_span(keepalive_actors))) {
            return -1;
          }
        }
        return 0;
      },
      [this](atframework::atapp::app &, atframework::atapp::etcd_cluster_holder &) {
...
LIBATAPP_MACRO_API int service_discovery_module::add_service_discovery_cluster_context(
  ...
  int ret = get_app()->get_etcd_module()->init_cluster(
      ...
      [this, context](atframework::atapp::app &app, atframework::atapp::etcd_cluster_holder &cluster_holder,
                      bool init) -> int {
        int result = init_keepalives(*context);
        if (result != 0) {
          return result;
        }
        result = init_watchers(*context);
        if (result != 0) {
          return result;
        }

        if (init) {
          const std::list<etcd_keepalive::ptr_t> *keepalive_actors[] = {&context->internal_discovery_keepalive_actors_,
                                                                        &context->internal_topology_keepalive_actors_};
      ...
      [context](atframework::atapp::app &, atframework::atapp::etcd_cluster_holder &) {
        context->reset_internal_watchers_and_keepalives();
      });

  if (ret != 0) {
    return ret;
  }
  cluster_context_list_.push_back(context);
  return 0;
}

LIBATAPP_MACRO_API service_discovery_module::service_discovery_cluster_context::service_discovery_cluster_context()
    : etcd_cluster_holder_(nullptr),
...
LIBATAPP_MACRO_API int service_discovery_module::stop() {
  cluster_context_.reset_internal_watchers_and_keepalives();
  for (std::shared_ptr<service_discovery_cluster_context> context : cluster_context_list_) {
    context->reset_internal_watchers_and_keepalives();
  }
  return 0;
}
...
void service_discovery_module::update_keepalive_topology_value() {
  ...
  if (new_value != internal_keepalive_topology_value_) {
    ...
    for (std::list<etcd_keepalive::ptr_t>::iterator iter = cluster_context_.internal_topology_keepalive_actors_.begin();
         iter != cluster_context_.internal_topology_keepalive_actors_.end(); ++iter) {
      if (*iter) {
    ...
    for (std::shared_ptr<service_discovery_cluster_context> context : cluster_context_list_) {
      for (std::list<etcd_keepalive::ptr_t>::iterator iter = context->internal_topology_keepalive_actors_.begin();
           iter != context->internal_topology_keepalive_actors_.end(); ++iter) {
        if (*iter) {
          (*iter)->set_value(internal_keepalive_topology_value_);
        }
      }
...
void service_discovery_module::update_keepalive_discovery_value() {
  ...
  if (new_value != internal_keepalive_discovery_value_) {
    ...
    for (std::list<etcd_keepalive::ptr_t>::iterator iter =
             cluster_context_.internal_discovery_keepalive_actors_.begin();
         iter != cluster_context_.internal_discovery_keepalive_actors_.end(); ++iter) {
      if (*iter) {
    ...
    for (std::shared_ptr<service_discovery_cluster_context> context : cluster_context_list_) {
      for (std::list<etcd_keepalive::ptr_t>::iterator iter = context->internal_discovery_keepalive_actors_.begin();
           iter != context->internal_discovery_keepalive_actors_.end(); ++iter) {
        if (*iter) {
          (*iter)->set_value(internal_keepalive_discovery_value_);
        }
      }
...
int service_discovery_module::init_keepalives(service_discovery_cluster_context &context) {
  // 先刷新topology数据，后刷新discovery数据，以保证策略路由变化时已获取到最新的topology信息
  update_keepalive_topology_value();
  update_keepalive_discovery_value();

  int ret = init_discovery_keepalive_by_id(context, internal_keepalive_discovery_value_);
  if (ret != 0) {
    return ret;
  }
  ret = init_discovery_keepalive_by_name(context, internal_keepalive_discovery_value_);
  if (ret != 0) {
    return ret;
  }
  ret = init_topology_keepalive(context, internal_keepalive_topology_value_);
  if (ret != 0) {
    return ret;
  }
...
LIBATAPP_MACRO_API int service_discovery_module::init_discovery_keepalive_by_id(
  ...
  atapp::etcd_keepalive::ptr_t actor =
      add_keepalive_actor(*context.etcd_cluster_holder_, value,
                          get_discovery_by_id_path(context.etcd_cluster_holder_->get_configure_path()));
  if (!actor) {
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(context.etcd_cluster_holder_->get_etcd_cluster(),
                                          "create etcd_keepalive for by_id index failed.");
    return -1;
  }

  context.internal_discovery_keepalive_actors_.push_back(actor);
  LIBATAPP_MACRO_ETCD_CLUSTER_LOG_INFO(context.etcd_cluster_holder_->get_etcd_cluster(),
...
LIBATAPP_MACRO_API int service_discovery_module::init_discovery_keepalive_by_name(
  ...
  atapp::etcd_keepalive::ptr_t actor =
      add_keepalive_actor(*context.etcd_cluster_holder_, value,
                          get_discovery_by_name_path(context.etcd_cluster_holder_->get_configure_path()));
  if (!actor) {
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(context.etcd_cluster_holder_->get_etcd_cluster(),
...
LIBATAPP_MACRO_API int service_discovery_module::init_topology_keepalive(service_discovery_cluster_context &context,
                                                                         std::string &value) {
  atapp::etcd_keepalive::ptr_t actor = add_keepalive_actor(
      *context.etcd_cluster_holder_, value, get_topology_path(context.etcd_cluster_holder_->get_configure_path()));
  if (!actor) {
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(context.etcd_cluster_holder_->get_etcd_cluster(),
  ...
  context.internal_topology_keepalive_actors_.push_back(actor);
  LIBATAPP_MACRO_ETCD_CLUSTER_LOG_INFO(context.etcd_cluster_holder_->get_etcd_cluster(),
                                       "create etcd_keepalive {} for topology index {} success",
                                       reinterpret_cast<const void *>(actor.get()),
                                       get_topology_path(context.etcd_cluster_holder_->get_configure_path()));
  return 0;
}

int service_discovery_module::init_watchers(service_discovery_cluster_context &context) {
  int ret = init_discovery_watcher_by_id(context);
  if (ret != 0) {
    return ret;
  }
  ret = init_discovery_watcher_by_name(context);
  if (ret != 0) {
    return ret;
  }
  ret = init_topology_watcher(context);
  if (ret != 0) {
    return ret;
  }
  return 0;
...
LIBATAPP_MACRO_API std::string service_discovery_module::get_discovery_by_id_path(const std::string &path) const {
  const atframework::atapp::app *owner = get_app();
  if (nullptr == owner) {
    return std::string();
  }

  return LOG_WRAPPER_FWAPI_FORMAT("{}{}/{}-{}", path, ETCD_MODULE_BY_ID_DIR, owner->get_app_name(), owner->get_id());
}

LIBATAPP_MACRO_API std::string service_discovery_module::get_discovery_by_name_path(const std::string &path) const {
  const atframework::atapp::app *owner = get_app();
  if (nullptr == owner) {
    return std::string();
  }

  return LOG_WRAPPER_FWAPI_FORMAT("{}{}/{}-{}", path, ETCD_MODULE_BY_NAME_DIR, owner->get_app_name(), owner->get_id());
}

LIBATAPP_MACRO_API std::string service_discovery_module::get_topology_path(const std::string &path) const {
  const atframework::atapp::app *owner = get_app();
  if (nullptr == owner) {
    return std::string();
  }

  return LOG_WRAPPER_FWAPI_FORMAT("{}{}/{}-{}", path, ETCD_MODULE_TOPOLOGY_DIR, owner->get_app_name(), owner->get_id());
}
...
LIBATAPP_MACRO_API std::string service_discovery_module::get_topology_watcher_path(const std::string &path) const {
...
LIBATAPP_MACRO_API int service_discovery_module::init_discovery_watcher_by_id(
  ...
  if (!context.internal_discovery_watcher_by_id_) {
    std::string watch_path =
        LOG_WRAPPER_FWAPI_FORMAT("{}{}", context.etcd_cluster_holder_->get_configure_path(), ETCD_MODULE_BY_ID_DIR);

    context.internal_discovery_watcher_by_id_ =
        atapp::etcd_watcher::create(context.etcd_cluster_holder_->get_etcd_cluster(), watch_path, "+1");
    if (!context.internal_discovery_watcher_by_id_) {
      LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(context.etcd_cluster_holder_->get_etcd_cluster(),
    ...
    context.internal_discovery_watcher_by_id_->set_conf_from_protobuf(
        context.etcd_cluster_holder_->get_configure().watcher());
    context.etcd_cluster_holder_->get_etcd_cluster().add_watcher(context.internal_discovery_watcher_by_id_);
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_INFO(context.etcd_cluster_holder_->get_etcd_cluster(),
                                         "create etcd_watcher for by_id index {} success", watch_path);

    context.internal_discovery_watcher_by_id_->set_evt_handle(
        discovery_watcher_callback_list_wrapper_t(*this, context, context.discovery_watcher_by_id_callbacks_,
                                                  ++context.discovery_watcher_snapshot_index_allocator_));
  }
  return 0;
}
...
LIBATAPP_MACRO_API int service_discovery_module::init_discovery_watcher_by_name(
    service_discovery_cluster_context &context) {
  if (!context.internal_discovery_watcher_by_name_) {
    // generate watch data
    std::string watch_path =
        LOG_WRAPPER_FWAPI_FORMAT("{}{}", context.etcd_cluster_holder_->get_configure_path(), ETCD_MODULE_BY_NAME_DIR);

    context.internal_discovery_watcher_by_name_ =
        atapp::etcd_watcher::create(context.etcd_cluster_holder_->get_etcd_cluster(), watch_path, "+1");
    if (!context.internal_discovery_watcher_by_name_) {
      LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(context.etcd_cluster_holder_->get_etcd_cluster(),
    ...
    context.internal_discovery_watcher_by_name_->set_conf_from_protobuf(
        context.etcd_cluster_holder_->get_configure().watcher());
    context.etcd_cluster_holder_->get_etcd_cluster().add_watcher(context.internal_discovery_watcher_by_name_);
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_INFO(context.etcd_cluster_holder_->get_etcd_cluster(),
                                         "create etcd_watcher for by_name index {} success", watch_path);

    context.internal_discovery_watcher_by_name_->set_evt_handle(
        discovery_watcher_callback_list_wrapper_t(*this, context, context.discovery_watcher_by_name_callbacks_,
                                                  ++context.discovery_watcher_snapshot_index_allocator_));
  }
  return 0;
}

LIBATAPP_MACRO_API int service_discovery_module::init_topology_watcher(service_discovery_cluster_context &context) {
  if (!context.internal_topology_watcher_) {
    // generate watch data
    std::string watch_path =
        LOG_WRAPPER_FWAPI_FORMAT("{}{}", context.etcd_cluster_holder_->get_configure_path(), ETCD_MODULE_TOPOLOGY_DIR);

    context.internal_topology_watcher_ =
        atapp::etcd_watcher::create(context.etcd_cluster_holder_->get_etcd_cluster(), watch_path, "+1");
    if (!context.internal_topology_watcher_) {
      LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(context.etcd_cluster_holder_->get_etcd_cluster(),
    ...
    context.internal_topology_watcher_->set_conf_from_protobuf(context.etcd_cluster_holder_->get_configure().watcher());
    context.etcd_cluster_holder_->get_etcd_cluster().add_watcher(context.internal_topology_watcher_);
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_INFO(context.etcd_cluster_holder_->get_etcd_cluster(),
                                         "create etcd_watcher for topology index {} success", watch_path);

    context.internal_topology_watcher_->set_evt_handle(topology_watcher_callback_list_wrapper_t(
        *this, context, context.topology_watcher_callbacks_, ++context.topology_watcher_snapshot_index_allocator_));
  }
  return 0;
}
...
LIBATAPP_MACRO_API void service_discovery_module::add_discovery_watcher_by_id_callback(
    const discovery_watcher_list_callback_t &fn) {
  cluster_context_.add_discovery_watcher_by_id_callback(fn);
}

LIBATAPP_MACRO_API void service_discovery_module::add_discovery_watcher_by_name_callback(
    const discovery_watcher_list_callback_t &fn) {
  cluster_context_.add_discovery_watcher_by_name_callback(fn);
}

LIBATAPP_MACRO_API void service_discovery_module::add_topology_watcher_callback(
    const topology_watcher_list_callback_t &fn) {
...
service_discovery_module::get_last_etcd_event_topology_header() const noexcept {
  return cluster_context_.last_etcd_event_topology_header_;
}

LIBATAPP_MACRO_API const ::atframework::atapp::etcd_response_header &
service_discovery_module::get_last_etcd_event_discovery_header() const noexcept {
  return cluster_context_.last_etcd_event_discovery_header_;
}

LIBATAPP_MACRO_API const atapp::protocol::atapp_etcd &service_discovery_module::get_configure() const {
  return get_app()->get_origin_configure().service_discovery_etcd();
}

LIBATAPP_MACRO_API const std::string &service_discovery_module::get_configure_path() const {
  return cluster_context_.etcd_cluster_holder_->get_configure_path();
}

LIBATAPP_MACRO_API atapp::etcd_keepalive::ptr_t service_discovery_module::add_keepalive_actor(
    etcd_cluster_holder &cluster_holder, std::string &val, const std::string &node_path) {
  ...
  ret = atapp::etcd_keepalive::create(cluster_holder.get_etcd_cluster(), node_path);
  if (!ret) {
    LIBATAPP_MACRO_ETCD_CLUSTER_LOG_ERROR(cluster_holder.get_etcd_cluster(), "create etcd_keepalive failed.");
    return ret;
  }
  ...
  if (!cluster_holder.get_etcd_cluster().add_keepalive(ret)) {
    ret.reset();
  }
...
LIBATAPP_MACRO_API atapp::etcd_keepalive::ptr_t service_discovery_module::add_keepalive_actor_default(
    std::string &val, const std::string &node_path) {
  return add_keepalive_actor(*cluster_context_.etcd_cluster_holder_, val, node_path);
}

LIBATAPP_MACRO_API bool service_discovery_module::remove_keepalive_actor(
    etcd_cluster_holder &cluster_holder, const atapp::etcd_keepalive::ptr_t &keepalive) {
  if (!keepalive) {
    return false;
  }

  return cluster_holder.get_etcd_cluster().remove_keepalive(keepalive);
}

LIBATAPP_MACRO_API bool service_discovery_module::remove_keepalive_actor_default(
    const atapp::etcd_keepalive::ptr_t &keepalive) {
  std::list<etcd_keepalive::ptr_t> *keepalive_actors[] = {&cluster_context_.internal_discovery_keepalive_actors_,
                                                          &cluster_context_.internal_topology_keepalive_actors_};
  ...
  return remove_keepalive_actor(*cluster_context_.etcd_cluster_holder_, keepalive);
}
...
LIBATAPP_MACRO_API bool service_discovery_module::has_discovery_snapshot() const noexcept {
  return !cluster_context_.discovery_watcher_snapshot_index_.empty();
}
...
LIBATAPP_MACRO_API bool service_discovery_module::has_topology_snapshot() const noexcept {
  return !cluster_context_.topology_watcher_snapshot_index_.empty();
}
...
service_discovery_module::create_discovery_watcher_callback_list_wrapper(service_discovery_cluster_context &context) {
  return std::make_unique<etcd_watcher::watch_event_fn_t>(
      discovery_watcher_callback_list_wrapper_t(*this, context, ++context.discovery_watcher_snapshot_index_allocator_));
}
LIBATAPP_MACRO_API etcd_watcher::watch_event_fn_t_ptr
service_discovery_module::create_topology_watcher_callback_list_wrapper(service_discovery_cluster_context &context) {
  return std::make_unique<etcd_watcher::watch_event_fn_t>(
      topology_watcher_callback_list_wrapper_t(*this, context, ++context.topology_watcher_snapshot_index_allocator_));
}
...
service_discovery_module::discovery_watcher_callback_list_wrapper_t::discovery_watcher_callback_list_wrapper_t(
    ...
    : mod(&m), ctx(&c), callbacks(nullptr), snapshot_index(index), has_insert_snapshot_index(false) {}

service_discovery_module::discovery_watcher_callback_list_wrapper_t::discovery_watcher_callback_list_wrapper_t(
    service_discovery_module &m, service_discovery_cluster_context &c,
    std::list<discovery_watcher_list_callback_t> &cbks, int64_t index)
    : mod(&m), ctx(&c), callbacks(&cbks), snapshot_index(index), has_insert_snapshot_index(false) {}

service_discovery_module::discovery_watcher_callback_list_wrapper_t::~discovery_watcher_callback_list_wrapper_t() {
  if (nullptr != ctx && 0 != snapshot_index && has_insert_snapshot_index) {
    ctx->discovery_watcher_snapshot_index_.erase(snapshot_index);
  }
}
...
void service_discovery_module::discovery_watcher_callback_list_wrapper_t::operator()(
  ...
  ctx->last_etcd_event_discovery_header_ = header;

  bool enable_snapshot = body.snapshot && 0 != snapshot_index;
  if (enable_snapshot) {
    if (!has_insert_snapshot_index) {
      ctx->discovery_watcher_snapshot_index_.insert(snapshot_index);
    ...
    if (!ctx->discovery_watcher_snapshot_index_.empty() &&
        *ctx->discovery_watcher_snapshot_index_.begin() != snapshot_index) {
...
service_discovery_module::topology_watcher_callback_list_wrapper_t::~topology_watcher_callback_list_wrapper_t() {
  if (nullptr != ctx && 0 != snapshot_index && has_insert_snapshot_index) {
    ctx->topology_watcher_snapshot_index_.erase(snapshot_index);
  }
}
...
void service_discovery_module::topology_watcher_callback_list_wrapper_t::operator()(
  ...
  ctx->last_etcd_event_topology_header_ = header;

  bool enable_snapshot = body.snapshot && 0 != snapshot_index;
  if (enable_snapshot) {
    if (!has_insert_snapshot_index) {
      ctx->topology_watcher_snapshot_index_.insert(snapshot_index);
      has_insert_snapshot_index = true;
    }

    // We just accept the earliest watcher as snapshot notifier.
    // So just enable by_id or by_name when initializing and they will has the smallest index.
    if (!ctx->topology_watcher_snapshot_index_.empty() &&
        *ctx->topology_watcher_snapshot_index_.begin() != snapshot_index) {
      enable_snapshot = false;
    }
  }
  ...
  for (size_t i = 0; i < body.events.size(); ++i) {
    ...
    topology_info.context_ptr = reinterpret_cast<uintptr_t>(ctx);
...
void service_discovery_module::service_discovery_cluster_context::reset_internal_watchers_and_keepalives() {
  if (internal_discovery_watcher_by_name_) {
    etcd_cluster_holder_->get_etcd_cluster().remove_watcher(internal_discovery_watcher_by_name_);
    internal_discovery_watcher_by_name_.reset();
  }

  if (internal_discovery_watcher_by_id_) {
    etcd_cluster_holder_->get_etcd_cluster().remove_watcher(internal_discovery_watcher_by_id_);
    internal_discovery_watcher_by_id_.reset();
  }

  if (internal_topology_watcher_) {
    etcd_cluster_holder_->get_etcd_cluster().remove_watcher(internal_topology_watcher_);
