    ...
    if (now_min != stat_.last_checkpoint_min) {
      ...
      if (last_min + 1 == now_min) {
        ...
        if (inner_module_etcd_) {
          ...
          FWLOGINFO(
              "\tetcd module(last minite): request count: {}, error request: {}, continue error: {}, success request: "
          ...
          FWLOGINFO(
              "\tetcd module(sum): request count: {}, error request: {}, continue error: {}, success request: "
              "{}, continue success request {}",
...
int app::apply_configure() {
...

  {
    ...
    ::util::hash::murmur_hash3_x64_128(conf_.origin.name().c_str(), static_cast<int>(conf_.origin.name().size()),
                                       0x01000193U, hash_out);
    char hash_code_str[40] = {0};
    UTIL_STRFUNC_SNPRINTF(hash_code_str, sizeof(hash_code_str), "%016llX%016llX",
                          static_cast<unsigned long long>(hash_out[0]), static_cast<unsigned long long>(hash_out[1]));
  ...
  if (!inner_module_etcd_) {
    ...
    for (size_t i = start_idx; i < nodes.size() && (0 == end_idx || i < end_idx); ++i) {
      ...
      add_custom_command_rsp(
          params, LOG_WRAPPER_FWAPI_FORMAT("node -> private data: {}, destroy event: {}, hash: {:x}{:x}, {}",
...
int app::send_last_command(uv_loop_t *ev_loop) {
  ...
  if (NULL == ep) {
    close_timer(tick_timer_.timeout_timer);
    ss() << util::cli::shell_font_style::SHELL_FONT_COLOR_RED << "connect to " << use_addr.address << " timeout."
         << std::endl;
