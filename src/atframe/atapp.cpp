...
#  endif
  ...
  internal_module_service_discovery_ = std::make_shared<atapp::service_discovery_module>();
  add_module(internal_module_service_discovery_);
...
// Ignore SIGPIPE
  ...
  ret = setup_curl_multi();
  if (ret < 0) {
    FWLOGERROR("setup curl multi failed");
    bus_node_.reset();
    write_startup_error_file(ret);
    return setup_result_ = ret;
  }
...
LIBATAPP_MACRO_API int app::run_once(uint64_t min_event_count, std::chrono::system_clock::duration timeout_duration) {
  ...
  do {
    ...
    if (timeout_duration.count() > 0) {
      atfw::util::time::time_utility::update();
      if (timeout <= atfw::util::time::time_utility::sys_now()) {
        break;
      }
    }
...
LIBATAPP_MACRO_API int app::reload() {
  ...
  if (is_running()) {
    ...
    if (internal_module_service_discovery_) {
      internal_module_service_discovery_->set_maybe_update_keepalive_topology_value();
      internal_module_service_discovery_->set_maybe_update_keepalive_discovery_value();
    }
  }
...
LIBATAPP_MACRO_API int app::tick() {
  ...
  // stat log
  do {
    time_t now_min = atfw::util::time::time_utility::get_sys_now() / atfw::util::time::time_utility::MINITE_SECONDS;
    ...
    if (last_min + 1 == now_min) {
...
#ifdef WIN32
      FWLOGINFO("[STATISTICS]: {} CPU usage: user {:02.3}%, sys {:02.3}%, max rss: {}{}, page faults: {}",
                get_app_name(),
                offset_usr /
                    (static_cast<float>(atfw::util::time::time_utility::MINITE_SECONDS) * 10000.0f),  // usec and add %
                offset_sys /
                    (static_cast<float>(atfw::util::time::time_utility::MINITE_SECONDS) * 10000.0f),  // usec and add %
      ...
      if (internal_module_service_discovery_) {
        const ::atframework::atapp::etcd_cluster::stats_t &current =
            internal_module_service_discovery_->get_raw_etcd_ctx().get_stats();
        FWLOGINFO(
      ...
      if (internal_module_service_discovery_) {
        stats_.internal_etcd = internal_module_service_discovery_->get_raw_etcd_ctx().get_stats();
      }
    }
...
LIBATAPP_MACRO_API atfw::util::cli::cmd_option_ci::ptr_type app::get_command_manager() {
  if (!cmd_handler_) {
    return cmd_handler_ = atfw::util::cli::cmd_option_ci::create();
  }

  return cmd_handler_;
}

LIBATAPP_MACRO_API atfw::util::cli::cmd_option::ptr_type app::get_option_manager() {
  if (!app_option_) {
    return app_option_ = atfw::util::cli::cmd_option::create();
  }
...
LIBATAPP_MACRO_API atfw::util::network::http_request::curl_m_bind_ptr_t app::get_shared_curl_multi_context()
    const noexcept {
  return curl_multi_;
}
...
LIBATAPP_MACRO_API void app::parse_configures_into(ATBUS_MACRO_PROTOBUF_NAMESPACE_ID::Message &dst,
  ...
  if (!configure_prefix_path.empty()) {
    atfw::util::config::ini_value::ptr_t cfg_value =
        cfg_loader_.get_root_node().get_child_by_path(configure_prefix_path);
    if (cfg_value) {
...
LIBATAPP_MACRO_API int32_t app::get_runtime_stateful_pod_index() const noexcept {
  ...
  if (find_res == std::string::npos || find_res + 1 >= node_name.size()) {
    ...
    const_cast<app *>(this)->conf_.runtime_pod_stateful_index =
        atfw::util::string::to_int<int32_t>(node_name.c_str() + find_res);
  }
...
LIBATAPP_MACRO_API atapp::protocol::atapp_area &app::mutable_area() {
  if (internal_module_service_discovery_) {
    internal_module_service_discovery_->set_maybe_update_keepalive_discovery_area();
  }
  return *conf_.origin.mutable_area();
}

LIBATAPP_MACRO_API atfw::util::time::time_utility::raw_time_t::duration app::get_configure_message_timeout()
    const noexcept {
  ...
  atfw::util::time::time_utility::raw_time_t::duration ret =
...
LIBATAPP_MACRO_API void app::pack(atapp::protocol::atapp_topology_info &out) const {
  ...
  if (bus_node_ && bus_node_->get_upstream_endpoint() != nullptr) {
    out.set_upstream_id(bus_node_->get_upstream_endpoint()->get_id());
  }

  if (conf_.origin.bus().topology().has_data()) {
    *out.mutable_data() = conf_.origin.bus().topology().data();
  }
}
...
app::get_service_discovery_module() const noexcept {
  return internal_module_service_discovery_;
}
...
LIBATAPP_MACRO_API const etcd_discovery_set &app::get_global_discovery() const noexcept {
  if (internal_module_service_discovery_) {
    return internal_module_service_discovery_->get_global_discovery();
  }
...
LIBATAPP_MACRO_API etcd_discovery_node::ptr_t app::get_discovery_node_by_id(uint64_t id) const noexcept {
  ...
  return internal_module_service_discovery_->get_global_discovery().get_node_by_id(id);
}
...
LIBATAPP_MACRO_API etcd_discovery_node::ptr_t app::get_discovery_node_by_name(const std::string &name) const noexcept {
  ...
  return internal_module_service_discovery_->get_global_discovery().get_node_by_name(name);
}
...
LIBATAPP_MACRO_API int32_t app::send_message(uint64_t target_node_id, int32_t type, gsl::span<const unsigned char> data,
  ...
  do {
    ...
    etcd_discovery_node::ptr_t node =
        internal_module_service_discovery_->get_global_discovery().get_node_by_id(target_node_id);
    if (!node) {
      break;
    }
...
LIBATAPP_MACRO_API int32_t app::send_message(const std::string &target_node_name, int32_t type,
  ...
  etcd_discovery_node::ptr_t node =
      internal_module_service_discovery_->get_global_discovery().get_node_by_name(target_node_name);
  if (!node) {
    return EN_ATBUS_ERR_ATNODE_NOT_FOUND;
  }
...
LIBATAPP_MACRO_API int32_t app::send_message_by_consistent_hash(gsl::span<const unsigned char> hash_buf, int32_t type,
  ...
  return send_message_by_consistent_hash(internal_module_service_discovery_->get_global_discovery(), hash_buf, type,
                                         data, msg_sequence, metadata);
}
...
LIBATAPP_MACRO_API int32_t app::send_message_by_consistent_hash(uint64_t hash_key, int32_t type,
  ...
  return send_message_by_consistent_hash(internal_module_service_discovery_->get_global_discovery(), hash_key, type,
                                         data, msg_sequence, metadata);
}
...
LIBATAPP_MACRO_API int32_t app::send_message_by_random(int32_t type, gsl::span<const unsigned char> data,
  ...
  return send_message_by_random(internal_module_service_discovery_->get_global_discovery(), type, data, msg_sequence,
                                metadata);
}
...
LIBATAPP_MACRO_API int32_t app::send_message_by_round_robin(int32_t type, gsl::span<const unsigned char> data,
  ...
  return send_message_by_round_robin(internal_module_service_discovery_->get_global_discovery(), type, data,
...
LIBATAPP_MACRO_API bool app::add_endpoint_waker(atfw::util::time::time_utility::raw_time_t wakeup_time,
  ...
  endpoint_waker_[std::pair<atfw::util::time::time_utility::raw_time_t, atapp_endpoint *>(wakeup_time, ep_ptr)] =
      ep_watcher;
  if (previous_time > wakeup_time) {
    endpoint_waker_.erase(
        std::pair<atfw::util::time::time_utility::raw_time_t, atapp_endpoint *>(previous_time, ep_ptr));
  }
  return true;
}
...
bool app::set_flag(flag_t f, bool v) {
  ...
  uint64_t mask_value = static_cast<uint64_t>(1) << static_cast<uint32_t>(f_value);
...
LIBATAPP_MACRO_API bool app::check_flag(flag_t f) const noexcept {
  ...
  return 0 != (flags_.load(std::memory_order_acquire) & (static_cast<uint64_t>(1) << static_cast<uint32_t>(f_value)));
}
...
void app::_app_setup_signal_handle(int signo) {
  ...
  for (int i = 0; i < static_cast<int>(internal_options_t::kMaxSignalCount); ++i) {
...
void app::process_signals() {
  ...
  int signals[static_cast<size_t>(internal_options_t::kMaxSignalCount)] = {0};
...
static void setup_load_category_from_environment(
  ...
  for (int32_t cat_default_index = 0;; ++cat_default_index) {
    std::string cat_prefix = atfw::util::log::format("{}_CATEGORY_{}", prefix, cat_default_index);
    std::transform(cat_prefix.begin(), cat_prefix.end(), cat_prefix.begin(), atfw::util::string::toupper<char>);

    std::string cat_name_key = atfw::util::log::format("{}_NAME", cat_prefix);
    std::string log_name = atfw::util::file_system::getenv(cat_name_key.c_str());
    if (log_name.empty()) {
      break;
    }
    ...
    if (cat_index_val.empty()) {
      ...
      cat_index = atfw::util::string::to_int<int32_t>(cat_index_val.c_str());
    }
    ...
    std::string exist_set_category_prefix = atfw::util::log::format("{}category.{}.", exist_set_prefix, cat_index);
    ...
    if (nullptr != dump_existed_set) {
      dump_existed_set->insert(atfw::util::log::format("{}index", exist_set_category_prefix));
    }

    // register log handles
    for (int32_t log_handle_index = 0;; ++log_handle_index) {
      std::string cat_handle_prefix = atfw::util::log::format("{}_{}_{}", prefix, log_name, log_handle_index);
      std::transform(cat_handle_prefix.begin(), cat_handle_prefix.end(), cat_handle_prefix.begin(),
                     atfw::util::string::toupper<char>);

      std::string cat_handle_type_key = atfw::util::log::format("{}_TYPE", cat_handle_prefix, log_handle_index);
      std::string sink_type = atfw::util::file_system::getenv(cat_handle_type_key.c_str());
      if (sink_type.empty()) {
        break;
      }
      ...
      std::string exist_set_sink_prefix =
          atfw::util::log::format("{}sink.{}.", exist_set_category_prefix, log_handle_index);
      if (nullptr != dump_existed_set) {
        dump_existed_set->insert(
...
void app::parse_environment_log_categories_into(atapp::protocol::atapp_log &dst,
  ...
  std::string level_value = atfw::util::file_system::getenv(env_level_name.c_str());
...
static void setup_load_category(
    const atfw::util::config::ini_value &src,
    ATBUS_MACRO_PROTOBUF_NAMESPACE_ID::RepeatedPtrField<atapp::protocol::atapp_log_category> &out,
    configure_key_set *dump_existed_set, gsl::string_view exist_set_prefix) {
  ...
  for (int32_t cat_default_index = 0;; ++cat_default_index) {
    auto cat_node = cat_array_node->second->get_children().find(atfw::util::log::format("{}", cat_default_index));
    ...
    // register log handles
    for (int32_t log_handle_index = 0;; ++log_handle_index) {
      auto cat_handle_node =
          cat_handles_node->second->get_children().find(atfw::util::log::format("{}", log_handle_index));
...
static void setup_load_category(
  ...
  if (index_node && index_node.IsScalar()) {
    if (!index_node.Scalar().empty()) {
      atfw::util::string::str2int(index, index_node.Scalar());
    }
  }
  ...
  index = log_cat->index();
  std::string exist_set_category_prefix = atfw::util::log::format("{}category.{}.", exist_set_prefix, index);
  if (nullptr != dump_existed_set) {
    dump_existed_set->insert(
        static_cast<std::string>(exist_set_category_prefix.substr(0, exist_set_category_prefix.size() - 1)));
  }
  ...
  if (sink_node.IsMap() && log_cat->sink_size() > old_sink_count) {
    std::string exist_set_sink_prefix = atfw::util::log::format("{}sink.", exist_set_category_prefix);
    if (nullptr != dump_existed_set) {
      dump_existed_set->insert(
          static_cast<std::string>(exist_set_sink_prefix.substr(0, exist_set_sink_prefix.size() - 1)));
    }
    ...
    for (int i = 0; i + old_sink_count < log_cat->sink_size(); ++i) {
      ...
      std::string exist_set_sink_prefix = atfw::util::log::format("{}sink.{}.", exist_set_category_prefix, i);
      if (nullptr != dump_existed_set) {
        dump_existed_set->insert(
            static_cast<std::string>(exist_set_sink_prefix.substr(0, exist_set_sink_prefix.size() - 1)));
      }
...
LIBATAPP_MACRO_API void app::trigger_event_on_discovery_event(etcd_discovery_action_t action,
...
LIBATAPP_MACRO_API void app::trigger_event_on_topology_event(
    etcd_watch_event action,
    const atfw::util::memory::strong_rc_ptr<atapp::protocol::atapp_topology_info> &topology_info,
    const etcd_data_version &) {
  if (!topology_info) {
    return;
  }

  atbus::topology_registry::ptr_t registry;
  if (bus_node_) {
    registry = bus_node_->get_topology_registry();
  }

  if (action == etcd_watch_event::kDelete) {
    // 不允许从注册表删除自己，有可能由网络抖动触发
    if (registry && topology_info->id() != get_app_id()) {
      registry->remove_peer(topology_info->id());
    }

    if (atbus_connector_ && topology_info->id() != get_app_id()) {
      atbus_connector_->remove_topology_peer(topology_info->id());
    }
  } else {
    atbus::topology_data::ptr_t topology_data_ptr{};
    if (registry || atbus_connector_) {
      if (topology_info->has_data()) {
    ...
    if (registry) {
      registry->update_peer(topology_info->id(), topology_info->upstream_id(), topology_data_ptr);
    }

    if (atbus_connector_) {
      atbus_connector_->update_topology_peer(topology_info->id(), topology_info->upstream_id(), topology_data_ptr);
    }
  }
}
...
int app::setup_signal() {
  ...
  if (!internal_setup_signal_action(SIGTERM, _app_setup_signal_handle)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGTERM", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  if (!internal_setup_signal_action(SIGINT, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGINT", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  ...
  if (!internal_setup_signal_action(SIGQUIT, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGQUIT", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  // lost parent process
  if (!internal_setup_signal_action(SIGHUP, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGHUP", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  // close stdin, stdout or stderr
  if (!internal_setup_signal_action(SIGPIPE, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGPIPE", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  // close tty
  if (!internal_setup_signal_action(SIGTSTP, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGTSTP", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  // tty input
  if (!internal_setup_signal_action(SIGTTIN, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGTTIN", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
  // tty output
  if (!internal_setup_signal_action(SIGTTOU, SIG_IGN)) {
    FWLOGERROR("setup signal {} failed, errno: {} -> {}", "SIGTTOU", atfw::util::platform::get_errno(),
               atfw::util::platform::get_strerrno(atfw::util::platform::get_errno(), gsl::make_span(errmsg)));
    ret = EN_ATAPP_ERR_SETUP_SIGNAL;
  }
#endif
...
void app::setup_startup_log() {
  ...
  if (wrapper.sink_size() == 0) {
    wrapper.add_sink(log_sink_maker::get_stdout_sink_reg()(wrapper, atfw::util::log::log_wrapper::categorize_t::DEFAULT,
...
int app::setup_curl_multi() {
  // init curl
  curl_multi_guard_ = create_curl_multi_guard();
  if (curl_multi_guard_ == nullptr) {
    FWLOGERROR("init curl multi guard failed");
    return -1;
  }
...
bool app::setup_timeout_timer(const ATBUS_MACRO_PROTOBUF_NAMESPACE_ID::Duration &duration) {
  ...
  if ATFW_UTIL_LIKELY_CONDITION (0 == res) {
...
bool app::cleanup_startup_error_file() {
  ...
  if (!atfw::util::file_system::is_exist(start_error_file.c_str())) {
    return true;
  }

  return atfw::util::file_system::remove(start_error_file.c_str());
}
...
LIBATAPP_MACRO_API bool app::add_custom_command_rsp(atfw::util::cli::callback_param params,
                                                    const std::string &rsp_text) {
...
LIBATAPP_MACRO_API app_id_t app::convert_app_id_by_string(const char *id_in, const std::vector<app_id_t> &mask_in) {
  ...
  if (id_in_is_number) {
    return ::atfw::util::string::to_int<app_id_t>(id_in);
  }
...
int app::prog_option_handler_upgrade_mode(atfw::util::cli::callback_param /*params*/) {
...
void app::setup_option(int argc, const char *argv[], void *priv_data) {
  ...
  atfw::util::cli::cmd_option::ptr_type opt_mgr = get_option_manager();
  ...
  if (conf_.app_version.empty()) {
    ...
    atfw::util::file_system::split_path(out, conf_.execute_path);
    std::stringstream ss;
    if (out.empty()) {
...
int app::command_handler_stop(atfw::util::cli::callback_param params) {
  char msg[256] = {0};
...
int app::command_handler_reload(atfw::util::cli::callback_param params) {
  bool enable_upgrade_mode = false;
  for (atfw::util::cli::cmd_option_list::size_type i = 0; i < params.get_params_number(); ++i) {
    if (params[i]->to_cpp_string() == "--upgrade") {
      enable_upgrade_mode = true;
...
int app::command_handler_list_discovery(atfw::util::cli::callback_param params) {
  if (!internal_module_service_discovery_) {
    const char *msg = "Service discovery module not initialized.";
    add_custom_command_rsp(params, msg);
  } else {
    size_t start_idx = 0;
    size_t end_idx = 0;
    ...
    const std::vector<etcd_discovery_node::ptr_t> &nodes =
        internal_module_service_discovery_->get_global_discovery().get_sorted_nodes();
...
void app::setup_command() {
  ...
  // enable discovery
  cmd_mgr->bind_cmd("enable-discovery", &app::command_handler_enable_discovery, this)
      ->set_help_msg("enable-discovery                            enable etcd discovery module.");
  // disable discovery
  cmd_mgr->bind_cmd("disable-discovery", &app::command_handler_disable_discovery, this)
      ->set_help_msg("disable-discovery                           disable etcd discovery module.");
