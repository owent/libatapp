...
Use this skill before writing or reviewing C++ in `libatapp`.
...
- Every public non-template API must use `LIBATAPP_MACRO_API` (or its matching `*_API` macro) on the declaration or
...
- Preserve generated protobuf code unless its source of truth is in scope.
