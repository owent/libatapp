...
class atapp_connector_atbus : public atapp_connector_impl {
 ...
 public:
  ...
  LIBATAPP_MACRO_API virtual uint32_t get_address_type(const atbus::channel::channel_address_t &addr) const
      UTIL_CONFIG_OVERRIDE;
  LIBATAPP_MACRO_API virtual int32_t on_start_listen(const atbus::channel::channel_address_t &addr)
      UTIL_CONFIG_OVERRIDE;
  LIBATAPP_MACRO_API virtual int32_t on_start_connect(
  ...
  LIBATAPP_MACRO_API virtual int32_t on_send_forward_request(
      atapp_connection_handle *handle, int32_t type, uint64_t *msg_sequence, const void *data, size_t data_size,
      const atapp::protocol::atapp_metadata *metadata) UTIL_CONFIG_OVERRIDE;

  LIBATAPP_MACRO_API virtual void on_discovery_event(etcd_discovery_action_t::type,
