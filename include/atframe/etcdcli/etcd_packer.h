...
namespace atapp {
    ...
    public:
        ...
        static LIBATAPP_MACRO_API void pack(const etcd_key_value &etcd_val, rapidjson::Value &json_val, rapidjson::Document &doc);
        static LIBATAPP_MACRO_API void unpack(etcd_key_value &etcd_val, const rapidjson::Value &json_val);

        static LIBATAPP_MACRO_API void pack(const etcd_response_header &etcd_val, rapidjson::Value &json_val, rapidjson::Document &doc);
         ...
         * @note if range_end = key+1(key="aa" and range_end="ab" or key="a\0xff" and range_end="b"), then the range is all keys prefixed
         * with key
         ...
         */
        static LIBATAPP_MACRO_API void pack_key_range(rapidjson::Value &json_val, const std::string &key, std::string range_end,
                                                      rapidjson::Document &doc);

        static LIBATAPP_MACRO_API void pack_string(rapidjson::Value &json_val, const char *key, const char *val, rapidjson::Document &doc);
