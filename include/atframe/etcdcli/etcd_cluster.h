...
namespace atapp {
    ...
    public:
        ...
        UTIL_FORCEINLINE void set_conf_authorization(const std::string &authorization) { conf_.authorization = authorization; }
        ...
        UTIL_FORCEINLINE const std::chrono::system_clock::duration &get_conf_http_timeout() const { return conf_.http_cmd_timeout; }
        ...
        UTIL_FORCEINLINE void set_conf_keepalive_timeout(std::chrono::system_clock::duration v) { conf_.keepalive_timeout = v; }
        UTIL_FORCEINLINE void set_conf_keepalive_timeout_sec(time_t v) { set_conf_keepalive_timeout(std::chrono::seconds(v)); }
        UTIL_FORCEINLINE const std::chrono::system_clock::duration &get_conf_keepalive_timeout() const { return conf_.keepalive_timeout; }
    ...
    public:
         ...
         * @param limit	        limit is a limit on the number of keys returned for the request. When limit is set to 0, it is treated as no
         * limit.
         * @param revision	    revision is the point-in-time of the key-value store to use for the range. If revision is less or equal to
         * zero, the range is over the newest key-value store. If the revision has been compacted, ErrCompacted is returned as a response.
         * @return http request
         */
        LIBATAPP_MACRO_API util::network::http_request::ptr_t
        create_request_kv_get(const std::string &key, const std::string &range_end = "", int64_t limit = 0, int64_t revision = 0);
         ...
         * @param prev_kv	    If prev_kv is set, etcd gets the previous key-value pair before changing it. The previous key-value pair
         * will be returned in the put response.
         * @param ignore_value	If ignore_value is set, etcd updates the key using its current value. Returns an error if the key does not
         * exist.
         * @param ignore_lease	If ignore_lease is set, etcd updates the key using its current lease. Returns an error if the key does not
         * exist.
         * @return http request
         */
        LIBATAPP_MACRO_API util::network::http_request::ptr_t create_request_kv_set(const std::string &key, const std::string &value,
                                                                                    bool assign_lease = false, bool prev_kv = false,
         ...
         * @param key	        key is the first key for the range. If range_end is not given, the request only looks up key.
         * @param range_end	    range_end is the upper bound on the requested range [key, range_end). just like etcd_packer::pack_key_range
         * @param prev_kv	    If prev_kv is set, etcd gets the previous key-value pairs before deleting it. The previous key-value pairs
         * will be returned in the delete response.
         * @return http request
         */
        LIBATAPP_MACRO_API util::network::http_request::ptr_t
        create_request_kv_del(const std::string &key, const std::string &range_end = "", bool prev_kv = false);
         ...
         * @param prev_kv	        If prev_kv is set, created watcher gets the previous KV before the event happens. If the previous KV is
         * already compacted, nothing will be returned.
         * @param progress_notify   progress_notify is set so that the etcd server will periodically send a WatchResponse with no events to
         * the new watcher if there are no recent events. It is useful when clients wish to recover a disconnected watcher starting from a
         * recent known revision. The etcd server may decide how often it will send notifications based on current load.
         * @return http request
         */
        LIBATAPP_MACRO_API util::network::http_request::ptr_t create_request_watch(const std::string &key,
                                                                                   const std::string &range_end = "",
                                                                                   int64_t start_revision = 0, bool prev_kv = false,
