...
├── test/                  # Unit tests
│   └── case/              # Test case files (111 test cases across 15 files)
...
│       ├── atapp_upstream_forward_test.cpp    # 8 tests (A.1–A.8): upstream proxy
│       ├── atapp_direct_connect_test.cpp      # 8 tests (B.1–B.8): direct peer topology
│       ├── atapp_downstream_send_test.cpp     # 4 tests (C.1–C.4): downstream send/pending
│       ├── atapp_topology_change_test.cpp     # 9 tests (D.1–D.9): topology change/recovery
│       ├── atapp_discovery_reconnect_test.cpp # 5 tests (E.1–E.5): discovery reconnect
│       ├── atapp_error_recovery_test.cpp      # 5 tests (F.1–F.5): error recovery
│       ├── atapp_discovery_test.cpp           # 12 tests: metadata, hash, round_robin
│       ├── atapp_etcd_cluster_test.cpp        # 18 tests: etcd client (req etcd)
│       ├── atapp_etcd_module_test.cpp         # 20 tests: etcd integration (req etcd)
│       ├── atapp_etcd_packer_test.cpp         # 7 tests: pack/unpack, base64
│       ├── atapp_configure_loader_test.cpp    # 6 tests: YAML/INI/env/expression
│       ├── atapp_worker_pool_test.cpp         # 5 tests: spawn, stop, foreach, tick
│       └── *.yaml / *.conf                    # Test configuration files (35 YAML, 2 .conf)
...
For command-line usage, Windows DLL/PATH notes, and etcd setup, see `.agents/skills/testing/SKILL.md`.
The table below focuses on **libatapp-specific** test inventory.
...
| `atapp_upstream_forward`    | `atapp_upstream_forward_test.cpp`    | 8     | Upstream proxy forwarding (3 nodes, A.1–A.8) |
| `atapp_direct_connect`      | `atapp_direct_connect_test.cpp`      | 8     | Direct peer topology (3 nodes, B.1–B.8)      |
| `atapp_downstream_send`     | `atapp_downstream_send_test.cpp`     | 4     | Downstream send/pending (C.1–C.4)            |
| `atapp_topology_change`     | `atapp_topology_change_test.cpp`     | 9     | Topology change/recovery (D.1–D.9)           |
| `atapp_discovery_reconnect` | `atapp_discovery_reconnect_test.cpp` | 5     | Discovery reconnect logic (E.1–E.5)          |
| `atapp_error_recovery`      | `atapp_error_recovery_test.cpp`      | 5     | Error recovery + cascade (F.1–F.5)           |
| `atapp_discovery`           | `atapp_discovery_test.cpp`           | 12    | Metadata filter, hash, round_robin, stress   |
| `atapp_etcd_cluster`        | `atapp_etcd_cluster_test.cpp`        | 18    | etcd client ops (requires etcd)              |
| `atapp_etcd_module`         | `atapp_etcd_module_test.cpp`         | 20    | etcd module integration (requires etcd)      |
...
Use `ci/etcd/setup-etcd.ps1` or `ci/etcd/setup-etcd.sh` to download/start/stop a local etcd instance for
`atapp_etcd_cluster` and `atapp_etcd_module`. Full command examples and options are documented in
...
| Testing | `.agents/skills/testing/SKILL.md` | Run and write unit tests (incl. Windows DLL/PATH notes, multi-node patterns, debug time control) |
| Configure Expression | `.agents/skills/configure-expression/SKILL.md` | Environment-variable expression expansion in config fields |
| Module & Connector | `.agents/skills/libatapp-module-connector/SKILL.md` | Module lifecycle, connector architecture, endpoint management, connection handles, message routing patterns |
| etcd & Discovery | `.agents/skills/libatapp-etcd-discovery/SKILL.md` | etcd client, service discovery sets, topology management, keepalive, watchers, etcd_module integration |
