...
if ( $RUN_MODE -eq "msvc.2019+.test" -or $RUN_MODE -eq "msvc.2019+.build" ) {
  ...
  & cmake ".." "-G" "$Env:CMAKE_GENERATOR" "-A" $Env:CMAKE_PLATFORM "-DBUILD_SHARED_LIBS=$ENV:BUILD_SHARED_LIBS"  `
    ...
    "-DATBUS_MACRO_ABORT_ON_PROTECTED_ERROR=ON" "-DCMAKE_SYSTEM_VERSION=$selectWinSDKVersion"                     `
    "-DATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_PROTOBUF_ALLOW_SHARED_LIBS=$ENV:BUILD_SHARED_LIBS" `
